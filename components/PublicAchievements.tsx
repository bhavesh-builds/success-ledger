...
export function PublicAchievements({ achievements }: PublicAchievementsProps) {
  ...
  const filteredAchievements = useMemo(() => {
    ...
    return achievements.filter((achievement) => {
      ...
      if (achievement.profiles?.full_name?.toLowerCase().includes(query)) {
        return true
      }
    ...
    <div className="py-6">
      <div className="mb-6">
        ...
        <p className="text-lg text-zinc-600 dark:text-zinc-400 mb-6">
          Discover inspiring accomplishments from the community
        </p>
        ...
        <div className="relative max-w-2xl">
          ...
          <input
            ...
            placeholder="Search achievements by content, category, tags, or author..."
                    ...
                    <div className="flex items-center gap-2 text-sm text-zinc-600 dark:text-zinc-400">
                      <div className="w-5 h-5 rounded-full bg-zinc-200 dark:bg-zinc-700 flex items-center justify-center">
                        <span className="text-xs font-medium text-zinc-600 dark:text-zinc-400">
                          {achievement.profiles.full_name.charAt(0).toUpperCase()}
                        </span>
                      </div>
                      <span>{achievement.profiles.full_name}</span>
                    </div>
                  )}
                </div>
